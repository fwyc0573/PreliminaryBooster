...
def measure_latency(pod_from, pod_to_IP):
    ...
    round_trip_line = next(line for line in resp.split('\n') if 'round-trip min/avg/max/stddev' in line)
...
def measure_bandwidth(pod_from, pod_to_IP):
    ...
    exec_command = ['/bin/sh', '-c', 'iperf3 -t 3 -c {} -l 2000B -f M'.format(pod_to_IP)]
...
def do_measuring_delay(pod_IPs, pod_nodes_mapping):
    ...
    master_pod = next(pod_name for pod_name in pod_nodes_mapping if "master" in pod_nodes_mapping[pod_name])
    record_delay_dict = {}
    for pod_name in pod_IPs:
        if pod_name != master_pod:
            # print(f"Now do measuring delay between {master_pod} -> {pod_name}...")
            record_delay_dict[pod_nodes_mapping[pod_name]] = measure_latency(master_pod, pod_IPs[pod_name])
...
def do_measuring_bandwidth(pod_IPs, pod_nodes_mapping):
    master_pod = next(pod_name for pod_name in pod_nodes_mapping if "master" in pod_nodes_mapping[pod_name])
    record_bandwidth_dict = {pod_nodes_mapping[pod_name]: {} for pod_name in pod_IPs if pod_name != master_pod}
            ...
            for pod_name_recv in pod_IPs:
                if pod_name_recv != master_pod and pod_name_recv != pod_name_send:
                    ...
                    record_bandwidth_dict[pod_nodes_mapping[pod_name_send]][
                        pod_nodes_mapping[pod_name_recv]] = measure_bandwidth(pod_name_send, pod_IPs[pod_name_recv])
...
def labeling():
    ...
    ping_pod_list = ["booster-measure-pod{}".format(i) for i in range(1, len(nodes) + 1)]
    ...
    for node in delay_dict:
        do_labeling(node, {**{"delay": delay_dict[node]},
                           **{"bandwidth_" + recv_node: bandwidth_dict[node][recv_node] for recv_node in
                              bandwidth_dict[node]}})
