...
class BoosterScheduler(object):
    ...
    def nodes_available(self):
        ...
        for n in self.v1.list_node().items:
            for status in n.status.conditions:
                ...
                ):  # also need to filter the master node
    ...
    def get_all_pods(self, kube_system=False):
        if not kube_system:
            return [
                x
                for x in self.v1.list_pod_for_all_namespaces(watch=False).items
                if x.metadata.namespace != "kube-system"
            ]
        else:
    ...
    def get_pods_on_node(self, node_name, kube_system=False):
        if not kube_system:
            return [
                ...
                for x in self.v1.list_pod_for_all_namespaces(watch=False).items
                if x.metadata.namespace != "kube-system"
                and x.spec.node_name == node_name
            ]
        else:
            return [
                x
                for x in self.v1.list_pod_for_all_namespaces(watch=False).items
                if x.spec.node_name == node_name
            ]
    ...
    def calculate_available_memory(self, node):
        ...
        sum_reserved_memory = sum(
            [self.get_pod_memory_request(y) for y in pods_on_node]
        )
    ...
    def calculate_available_res(self, node, res_type):
        pods_on_node = self.get_pods_on_node(node.metadata.name)
        sum_reserved_res = sum(
            [self.get_pod_res_request(y, res_type) for y in pods_on_node]
        )
        allocatable_res = self.convert_to_int(
            node.status.allocatable[res_type], res_type
        )
    ...
    def get_leastrequestedpriority_score(
        self,
        sum_reserved_cpu,
        capacity_cpu,
        sum_reserved_memory,
        ...
        return (
            ((capacity_cpu - sum_reserved_cpu) * 10 / capacity_cpu)
            + ((capacity_memory - sum_reserved_memory) * 10 / capacity_memory) / 2
            + ((capacity_disk - sum_reserved_disk) * 10 / capacity_disk) / 10
        )

    def get_balancedresourceallocation_score(
        self, cpuFraction, memoryFraction, diskFraction
    ):
        return (
            10
            - (
                abs(cpuFraction - memoryFraction)
                + abs(cpuFraction - diskFraction)
                + abs(memoryFraction - diskFraction) / 3
            )
            * 10
        )
    ...
    def get_layerlocalitypriority_score(self, node, pod_image_name):
        ...
        for each_image in images_list:
            ...
            if pod_image_name == result[0].get("RepoTags")[0]:
                print(
                    f"get_layerlocalitypriority_score | match RepoTags -> {pod_image_name}"
                )
                ...
                with self.orchestration_module._dist_sync_lock:
                    ...
                    layer_filter_list = (
                        [i[7:] for i in layer_list]
                        if "sha" in layer_list[0]
                        else layer_list
                    )
                    ...
                    for layer in self.orchestration_module.cluster_dist_dict[
                        node.metadata.name
                    ]["layer"]:
                        # 是需要的，且在缓存中
                        if (
                            layer in layer_filter_list
                            and self.orchestration_module.cluster_dist_dict[
                                node.metadata.name
                            ]["layer"][layer]["exit"]
                            == 1
                        ):
                            ...
                            hit_cache_sum_size += (
                                self.orchestration_module.cluster_dist_dict[
                                    node.metadata.name
                                ]["layer"][layer]["size"]
                            )
                    ...
                    sum_score = (
                        ...
                        + 5
                        * (
                            self.orchestration_module.orchestrator_layer_cache
                            - my_node_cache_now
                        )
                        / self.orchestration_module.orchestrator_layer_cache
                    )
                ...
                return (
                    sum_score,
                    {
                        ...
                        "image_id": image_id,
                        "lack_lf": {"layer": layer_filter_list, "file": []},
                    },
                    whole_layer_list_back,
                )
        return -1, None, None
    ...
    def get_cloud_node(self):
        return next(
            x
            for x in self.v1.list_node().items
            if "kubernetes.io/role" in x.metadata.labels.keys()
            and x.metadata.labels["kubernetes.io/role"] == "cloud"
        )

    def get_node_from_name(self, node_name):
        return next(
            x for x in self.v1.list_node().items if x.metadata.name == node_name
        )

    def get_node_from_podname_or_nodename(self, previous_element_name):
        if previous_element_name in [
            x.metadata.name for x in self.v1.list_node().items
        ]:
            return self.get_node_from_name(previous_element_name)
        else:
            return self.get_node_from_name(
                next(
                    x
                    for x in self.v1.list_pod_for_all_namespaces(watch=False).items
                    if previous_element_name in x.metadata.name
                ).spec.node_name
            )
    ...
    def get_nodes_in_radius(self, required_delay, required_bandwidth):
        ...
        delay_dict = {
            node: float(self.get_node_from_name(node).metadata.labels[x])
            for node in available_nodes
            for x in self.get_node_from_name(node).metadata.labels.keys()
            if "delay" in x
        }
        ...
        for node in available_nodes:
            bandwidth_dict[node] = round(
                sum(
                    [
                        int(self.get_node_from_name(node).metadata.labels[x])
                        for node in available_nodes
                        for x in self.get_node_from_name(node).metadata.labels.keys()
                        if "bandwidth_node" in x
                    ]
                )
        ...
        constrained_nodes_list = [
            node
            for node in delay_dict
            if required_delay > delay_dict[node]
            and required_bandwidth <= bandwidth_dict[node]
        ]
    ...
    def reused_placeholder_unused_pod_node(self, placeholder, nodes_enough_resource):
        covered_nodes = [
            self.get_node_from_podname_or_nodename(x) for x in placeholder.pods
        ]
        covered_node_names = [y.metadata.name for y in covered_nodes]
        if any(
            x.metadata.name not in covered_node_names + [placeholder.node]
            for x in nodes_enough_resource
        ):
            ...
            if n.metadata.name != placeholder.node:
                ...
                for pod in self.get_pods_on_node(n.metadata.name):
                    if pod.metadata.name.split("-")[0] in placeholder.pods:
                        placeholder_memory_matrix[
                            n.metadata.name
                        ] += self.get_pod_memory_request(pod)
        return placeholder_memory_matrix

    def reused_placeholder_used_pod_node(self, placeholder, pod, nodes_enough_resource):
        placeholder_memory_matrix = self.get_memory_matrix(
            placeholder, nodes_enough_resource
        )
        if any(
            placeholder_memory_matrix[x] + self.get_pod_memory_request(pod)
            <= placeholder.required_memory
            for x in placeholder_memory_matrix.keys()
        ):
            return True, next(
                x
                for x in nodes_enough_resource
                if x.metadata.name in placeholder_memory_matrix.keys()
    ...
    def assign_placeholder(self, pod, nodes_less_resource, nodes_enough_resource):
        ...
        for placeholder in placeholders_in_rad:
            is_any_usable, excluded_list = self.reused_placeholder_unused_pod_node(
                placeholder, nodes_enough_resource
            )
            if is_any_usable:
                self.add_pod_to_placeholder(pod, placeholder)
                return [
                    x.metadata.name not in excluded_list for x in nodes_enough_resource
                ]

        for placeholder in placeholders_in_rad:
            is_any_usable, chosen_node = self.reused_placeholder_used_pod_node(
                placeholder, pod, nodes_enough_resource
            )
            if is_any_usable:
                self.add_pod_to_placeholder(pod, placeholder)
                return [chosen_node]
        ...
        if len(nodes_enough_resource) > 1:
            ...
            self.add_pod_to_placeholder(
                pod, placeholder, self.get_pod_memory_request(pod)
            )
            return [
                x for x in nodes_enough_resource if x.metadata.name != placeholder.node
            ]
        else:
    ...
    def pod_has_placeholder(self, pod):
        try:
            ...
            return True, next(
                ...
                if pod.metadata.name.split("-")[0] in ph.pods
            )
        except StopIteration:
            return False, None
    ...
    def get_pod_res_request(self, pod, res_type):
        ...
        return sum(
            [
                self.convert_to_int(x.resources.requests[res_type], res_type)
                for x in pod.spec.containers
                if x.resources.requests is not None
            ]
        )
    ...
    def get_pod_memory_request(self, pod):
        return sum(
            [
                self.convert_to_int(x.resources.requests["memory"])
                for x in pod.spec.containers
                if x.resources.requests is not None
            ]
        )
    ...
    def get_pod_cpu_request(self, pod):
        return sum(
            [
                self.convert_to_int(x.resources.requests["cpu"])
                for x in pod.spec.containers
                if x.resources.requests is not None
            ]
        )
    ...
    def get_pod_disk_request(self, pod):
        return sum(
            [
                self.convert_to_int(x.resources.requests["ephemeral-storage"])
                for x in pod.spec.containers
                if x.resources.requests is not None
            ]
        )
    ...
    def narrow_nodes_by_capacity(self, pod, node_list):
        ...
        for n in node_list:
            if self.calculate_available_res(n, "memory") <= self.get_pod_res_request(
                pod, "memory"
            ):
                continue
            if self.calculate_available_res(n, "cpu") <= self.get_pod_res_request(
                pod, "cpu"
            ):
                continue
            if self.calculate_available_res(
                n, "ephemeral-storage"
            ) <= self.get_pod_res_request(pod, "ephemeral-storage"):
                continue
    ...
    def get_placeholder_by_pod(self, pod):
        try:
            return next(
                ...
                if pod.metadata.name.split("-")[0] in x.pods
            )
        except StopIteration:
            return None
    ...
    def get_reschedulable(self, node, new_memory_request):
        ...
        for old_pod in pods_on_node:
            ...
            if old_memory_request >= new_memory_request:
                old_start_point = next(
                    x for x in old_pod.metadata.labels.keys() if "delay_" in x
                ).split("_")[1]
                old_required_delay = int(
                    old_pod.metadata.labels["delay_" + old_start_point]
                )
                old_nodes_in_radius = self.narrow_nodes_by_capacity(
                    old_pod,
                    self.get_nodes_in_radius(old_start_point, old_required_delay),
                )
                ...
                if (
                    len(
                        [
                            x
                            for x in old_nodes_in_radius
                            if x.metadata.name != old_placeholder.node
                        ]
                    )
                    > 0
                ):
    ...
    def reschedule_pod(self, new_pod, new_nodes_in_radius):
        ...
        for n in new_nodes_in_radius:
            any_reschedulable, old_pod, reschedule_node = self.get_reschedulable(
                n, new_memory_request
            )
    ...
    def do_schedule(self, pod, namespace="default"):
        ...
        if len(nodes_enough_resource_in_rad):
            host_node, image_lack_dict, whole_layer_list_back = self.priority_schedule(
                pod, nodes_enough_resource_in_rad
            )  # merge_res_dict
            print(
                f"do_schedule | host_node = {host_node}, image_lack_dict = {image_lack_dict}"
            )
            ...
            if image_lack_dict["lack_lf"]["layer"] is not None:
                self.orchestration_module.orchestrate_deploy_trigger(
                    host_node, image_lack_dict, whole_layer_list_back
                )
    ...
    def priority_schedule(self, pod, pre_node_list, namespace="default"):
        ...
        """
        highest_score, host_node, image_lack_dict, whole_layer_list_back = (
            0,
            None,
            {},
            {},
        ...
        for node in pre_node_list:
            cpuFraction, sum_reserved_cpu, capacity_cpu = self.calculate_res_fraction(
                node, "cpu"
            )
            print(
                f"node -> {node.metadata.name} | priority_schedule | cpuFraction = {cpuFraction}, sum_reserved_cpu = {sum_reserved_cpu}, capacity_cpu = {capacity_cpu}"
            )
            ...
            (
                ...
                sum_reserved_memory,
                capacity_memory,
            ) = self.calculate_res_fraction(node, "memory")
            print(
                f"node -> {node.metadata.name} | priority_schedule | memoryFraction = {memoryFraction}, sum_reserved_memory = {sum_reserved_memory}, capacity_memory = {capacity_memory}"
            )
            ...
            (
                ...
                sum_reserved_disk,
                capacity_disk,
            ) = self.calculate_res_fraction(node, "ephemeral-storage")
            print(
                f"node -> {node.metadata.name} | priority_schedule | diskFraction = {diskFraction}, sum_reserved_disk = {sum_reserved_disk}, capacity_disk = {capacity_disk}"
            )

            score_LeastRequestedPriority = self.get_leastrequestedpriority_score(
                sum_reserved_cpu,
                capacity_cpu,
                sum_reserved_memory,
                capacity_memory,
            ...
            print(
                f"node -> {node.metadata.name} | priority_schedule | score_LeastRequestedPriority = {score_LeastRequestedPriority}"
            )

            score_BalancedResourceAllocation = (
                self.get_balancedresourceallocation_score(
                    cpuFraction, memoryFraction, diskFraction
                )
            )
            print(
                f"node -> {node.metadata.name} | priority_schedule | score_BalancedResourceAllocation = {score_BalancedResourceAllocation}"
            )

            with self.orchestration_module.share_sync_lock:
                (
                    score_LayerLocalityPriority,
                    image_lack_lf,
                    whole_layer_list_back,
                ) = self.get_layerlocalitypriority_score(
                    node, pod.spec.containers[0].image
                )
            print(
                f"node -> {node.metadata.name} | priority_schedule | score_LayerLocalityPriority = {score_LayerLocalityPriority}"
            )
            print(
                f"node -> {node.metadata.name} | priority_schedule | image_lack_lf = {image_lack_lf}"
            )

            node_final_score = (
                weighting_factor["LeastRequestedPriority"]
                * score_LeastRequestedPriority
                + weighting_factor["BalancedResourceAllocation"]
                * score_BalancedResourceAllocation
                + weighting_factor["LayerLocalityPriority"]
                * score_LayerLocalityPriority
            )

            print(
                f"node -> {node.metadata.name} | priority_schedule | final score = {node_final_score}"
            )

            if node_final_score > highest_score:
                highest_score, host_node, image_lack_dict = (
                    node_final_score,
                    node.metadata.name,
                    image_lack_lf,
                )
    ...
    def pre_schedule(self, pod, namespace="default"):
        ...
        print(
            f"pre_schedule | service required_delay = {required_delay}, required_bandwidth = {required_bandwidth}"
        )

        # Getting all the nodes inside the delay radius
        (
            all_measure_constrained_nodes,
            constrained_nodes_measure_dict,
        ) = self.get_nodes_in_radius(required_delay, required_bandwidth)
        print(
            f"pre_schedule | all_measure_constrained_nodes = {all_measure_constrained_nodes}"
        )
        print(
            f"pre_schedule | constrained_nodes_measure_dict = {constrained_nodes_measure_dict}"
        )

        nodes_enough_resource_in_rad = self.narrow_nodes_by_capacity(
            pod, [self.get_node_from_name(n) for n in all_measure_constrained_nodes]
        )
        print(
            f"pre_schedule | nodes_enough_resource_in_rad -> {[node.metadata.name for node in nodes_enough_resource_in_rad]}"
        )
    ...
    def schedule(self, pod, namespace="default"):
        try:
            ...
            if any_assigned_placeholder:
                ...
                previous_element_name = next(
                    x for x in pod.metadata.labels.keys() if "delay_" in x
                ).split("_")[1]
                # Get the delay constraint value from the labels
                required_delay = int(
                    pod.metadata.labels["delay_" + previous_element_name]
                )
                # Getting all the nodes inside the delay radius
                all_nodes_in_radius = self.get_nodes_in_radius(
                    previous_element_name, required_delay
                )
                nodes_enough_resource_in_rad = self.narrow_nodes_by_capacity(
                    pod, all_nodes_in_radius
                )
                    ...
                    if old_pod_name is not None:
                        ...
                        while (
                            len(
                                [
                                    x.metadata.name
                                    for x in self.get_all_pods()
                                    if old_pod_name.split("-")[0] in x.metadata.name
                                ]
                            )
                            > 1
                        ):
                ...
                if len(all_nodes_in_radius) > 1:
                    ...
                    for ph in self.placeholders:
                        print(
                            "INFO Placeholder on node: %s ;assigned Pods: %s\n"
                            % (ph.node, str(ph.pods))
                        )
    ...
    def patch_deployment(self, pod, node, namespace="default"):
        ...
        deployment = [
            x
            for x in extensions_v1beta1.list_deployment_for_all_namespaces(
                watch=False
            ).items
            if x.metadata.namespace == "default" and x.metadata.name == deployment_name
        ][-1]
