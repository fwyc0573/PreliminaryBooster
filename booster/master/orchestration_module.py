...
from utils_images import (
    init_system_metadata,
    record_service_pull_time,
    delete_k8s_service,
    layer_list_get,
    apply_k8s_service,
    shutil_remove_dirs,
...
def update_msg_encapsulate(
    local_to_del_dict={}, hub_to_get_dict={}, bro_to_send_dict={}, bro_get_check={}
):
    update_msg = {
        "local_to_del": local_to_del_dict,
    ...
    if operation == "add":  # "add" only used in master which aims to change "freq"
        for item in sync_dict[granularity][operation]:
            if item in my_dict[node_name][granularity]:
                ...
                my_dict[node_name][granularity][item] = {
                    "frq": 1,
                    "exit": 0,
                    "size": -1,
                }
        ...
        print(
            f"changes_to_dict | operation == 'size_update', sync_dict -> f{sync_dict}"
        )
        for item in sync_dict[granularity][operation]:
            if item in my_dict[node_name][granularity]:
                my_dict[node_name][granularity][item]["exit"] = 1
                my_dict[node_name][granularity][item]["size"] = sync_dict[granularity][
                    operation
                ][item]
                try:
...
class BoosterOrchestrationModule(object):
    def __init__(
        self,
        orchestrator_name="booster-orchestrator",
        host_master=os.getlogin(),
    ...
    def init_dist_in_memo(self):
        ...
        for each_node in os.listdir(node_dir_path):
            ...
            for layer in os.listdir(
                os.path.join(self.__menu_basic_path, "distribution", each_node, "layer")
            ):
                dist_dict[each_node]["layer"][layer] = {"frq": 0, "exit": 1, "size": -1}
            for file in os.listdir(
                os.path.join(self.__menu_basic_path, "distribution", each_node, "file")
            ):
                dist_dict[each_node]["file"][file] = {"frq": 0, "exit": 1, "size": -1}
            for image in os.listdir(
                os.path.join(self.__menu_basic_path, "distribution", each_node, "image")
            ):
    ...
    def init_random_collocation(self):
        ...
        for each_image in images_list:
            ...
            with open(manifest_path, "rb") as f:
                result = json.load(f)
            image_name = result[0].get(
                "RepoTags"
            )  # ['fengyicheng/offload_cloud:latest']
        ...
        for node in nodes_ip:
            if index == len(nodes_ip) + 1:
                my_list = alloc_layer_list[
                    index * num_mean : (index + 1) * num_mean
                    + len(alloc_layer_list) % len(nodes_ip)
                ]
            else:
            ...
            update_msg = update_msg_encapsulate(
                hub_to_get_dict={"layer": trans_pull_dict, "file": []}
            )
        ...
        for n in self.v1.list_node().items:
            for status in n.status.conditions:
                if (
                    ...
                    and "master" not in n.metadata.name
                ):
            ...
            for granularity in sync_dict:
                for operation in sync_dict[granularity]:
                    with self._dist_sync_lock:
                        self.__cluster_dist_dict = changes_to_dict(
                            self.__cluster_dist_dict,
                            sync_dict,
                            node_name,
                            granularity,
                            operation,
    ...
    def orchestrate_deploy_trigger(
        self, host_node, image_lack_lf, whole_layer_list_back
    ):
        self.push_lf_local_info(
            host_node,
            image_lack_lf["lack_lf"],
            image_lack_lf["image_id"],
            image_lack_lf["image_name"],
            whole_layer_list_back,
        )

    def push_lf_local_info(
        self, schedule_node, lack_dict, image_id, image_name, whole_layer_list_back
    ):
        ...
        with self.share_sync_lock:
                ...
                with self._dist_sync_lock:
                    for node in self.__cluster_dist_dict:
                        ...
                        if layer in self.__cluster_dist_dict[node]["layer"]:
                            if (
                                self.__cluster_dist_dict[node]["layer"][layer]["exit"]
                                == 1
                            ):
                                ...
                                pre_layer_node_dict[layer][node] = next(
                                    int(
                                        self.get_node_from_name(node).metadata.labels[x]
                                    )
                                    for x in self.get_node_from_name(
                                        node
                                    ).metadata.labels.keys()
                                    if ("bandwidth_" + schedule_node) in x
                                )
                                find_in_cluster = True
                if find_in_cluster:
                    select_node = max(
                        pre_layer_node_dict[layer], key=pre_layer_node_dict[layer].get
                    )
            ...
            for file in lack_dict["file"]:
                ...
                with self._dist_sync_lock:
                    for node in self.__cluster_dist_dict:
                        ...
                        if file in self.__cluster_dist_dict[node]["file"]:
                            if (
                                self.__cluster_dist_dict[node]["file"][file]["exit"]
                                == 1
                            ):
                ...
                if find_in_cluster:
                    select_node = max(
                        pre_file_node_dict[file], key=pre_file_node_dict[file].get
                    )
        ...
        res_schedule = proxy_push_schedule_node_notifier(
            nodes_ip_dict[schedule_node],
            "9000",
            metadata_path,
            metadata_list,
            lack_dict,
        ...
        for node in final_push_dict:
            if final_push_dict[node]["layer"] or final_push_dict[node]["file"]:
                res_dict[node] = proxy_push_node_notifier(
                    nodes_ip_dict[node],
                    "9000",
                    final_push_dict[node],
                    [nodes_ip_dict[schedule_node], "9000"],
                    image_id,
                    "deploy",
                )
        return res_dict
    ...
    def update_popularity(self, host_node, whole_layer_list_back, image_id):
        for layer in whole_layer_list_back:
            with self._dist_sync_lock:
                if layer not in self.cluster_dist_dict[host_node]["layer"]:
                    self.cluster_dist_dict[host_node]["layer"][layer] = {
                        "frq": 1,
                        "exit": 0,
                        "size": -1,
                    }
        ...
        with self._dist_sync_lock:
            if image_id not in self.cluster_dist_dict[host_node]["image"]:
                self.cluster_dist_dict[host_node]["image"][image_id] = {
                    "frq": 1,
                    "exit": 0,
                    "size": -1,
                }
    ...
    def delete_service_daemon(self):
        ...
        while True:
            ...
            dir_path = os.path.join(
                self.__menu_basic_path, "distribution", dict_info["from_node"], "image"
            )
            # 清除image信息
            with self._dist_sync_lock:
                try:
                    self.__cluster_dist_dict[dict_info["from_node"]]["image"][
                        dict_info["image_id"]
                    ]["exit"] = 0
                    os.rmdir(
                        os.path.join(dir_path, dict_info["image_id"])
                    )  # 删除分布数据库中文件夹
                except Exception as e:
                    print("delete_service_daemon | changes_to_dict miss key or ...", e)

            record_service_pull_time(
                node_name=dict_info["from_node"],
                image_id=dict_info["image_id"],
                image_name=dict_info["image_name"],
                pull_time=dict_info["pull_time"],
                hit_rate=dict_info["hit_rate"],
            )
            delete_k8s_service(
                node_name=dict_info["from_node"],
                image_id=dict_info["image_id"],
