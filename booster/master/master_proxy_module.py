...
def proxy_file_uploader(target_ip, target_port):
    ...
    res = requests.post(url=url, headers=header, files=[
...
def proxy_push_schedule_node_notifier(target_ip, target_port, basic_path, metadata_list, lack_dict, image_id, image_name,
                                      hub_to_get_dict):

    url = "http://" + target_ip + ":" + target_port + "/proxy_node/push_schedule_node_receiver"
    header = {
...
def proxy_push_node_notifier(target_ip, target_port, trans_dict, schedule_node_info, image_id, flow_type):
...
def proxy_changes_syncer_service_delete():
    ...
    orchestrator.service_delete_queue.put(
        {'from_node': from_node, 'image_id': image_id, 'image_name': image_name, 'pull_time': pull_time, 'hit_rate': hit_rate})
