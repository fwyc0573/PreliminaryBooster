...
def creat_manifest_file(config, resp, img_parts, img, tag, my_menu_path):
    ...
    for layer in resp.json()["layers"]:
        ...
        content[0]["Layers"].append(
            ublob + "/layer.tar"
        )  # 使用原始digest作为id序号而非再次进行sha256编码
...
def init_system_metadata(images_name_list, nodes_list, my_menu_path):
    ...
    for image in images_name_list:
        ...
        auth_head = get_auth_head(
            "application/vnd.docker.distribution.manifest.v2+json", repository
        )
        ...
        if not os.path.exists(my_image_metadata_path + config[7:]):
            ...
            # Create JSON file
            creat_json_file(
                config, registry, repository, auth_head, my_image_metadata_path
            )
            ...
            creat_manifest_file(
                config, resp, img_parts, img, tag, my_image_metadata_path
            )
        else:
            continue
...
def get_single_layer_from_hub(ublob, images_name, save_path, tmp_path):
    ublob = (
        "sha256:" + ublob if "sha256" not in ublob else ublob
    )  # "sha256:" is needed in this func
    layer_dir_construct = save_path + ublob[7:]
    if os.path.exists(layer_dir_construct) or os.path.exists(
        os.path.join(tmp_path[:-5], "layer", ublob[7:])
    ):
    ...
    return (
        int(os.path.getsize(save_path + ublob[7:] + "/layer.tar")) / 1024 / 1024
    )  # MB
...
def get_image_layer_from_hub(ublob_list, images_name, save_path):
    ...
    if len(need_ublob_list) == 0:
        ...
        return {
            "need_ublob_list": need_ublob_list,
            "save_path": save_path,
            "registry": registry,
...
def delete_k8s_service(node_name, image_id, image_name):
    pro = subprocess.Popen(
        "kubectl delete " + image_name + ".yaml", shell=True, stdout=subprocess.PIPE
    )
    pro.wait()
