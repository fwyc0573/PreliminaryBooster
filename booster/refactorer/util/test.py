...
def make_gz(source_dir, output_filename):
    ...
    with tarfile.open(output_filename, "w") as tar:  # "w:gz"
        ...
        tar.addfile(
            source_dir + "/",
            arcname=os.path.basename(output_filename.split(".")[0]),
            dereference=True,
        )
    return 0
...
def get_tree(tree, node_id, hash_addr, layer_addr, img):
    ...
    tree.create_node(
        ...
        data={"name": root_name, "size": 0, "hash": 0, "repeat": False, "img": img},
    )  # 根节点
    node_id += 1
        ...
        for one_file_dic in hash_dic[one_hash]:
            ...
            for one_dict in file_path:
                ...
                if one_dict not in children_list:
                    ...
                    pointer = [
                        node.identifier
                        for node in tree.children(pointer)
                        if node.tag == one_dict
                    ][0]
                    pass
    ...
    for one_layer in layer_dic:
        ...
        for cur_path in layer_dic[one_layer]:
            pointer = [
                node.identifier
                for node in tree.children(init_id)
                if node.tag == one_layer
            ][0]
...
def same_node_children(node1, node2, tree1):
    ...
    for child_index in range(len(node1_children_tag)):
        ...
        if (
            node1_children[child_index].data["hash"]
            != node2_children[child2_index].data["hash"]
        ):
            return False
        if (
            node1_children[child_index].data["size"]
            != node2_children[child2_index].data["size"]
        ):
            return False
    return True
...
def same_subtree(tree1):
    ...
    tt1 = time.time()
    hash_list = [
        str(node.data["hash"]) + str(node.data["size"]) + node.tag
        for node in tree1_leaf
    ]
    repeat_hash = {k: [] for k, v in Counter(hash_list).items()}

    for node in tree1_leaf:
        repeat_hash[str(node.data["hash"]) + str(node.data["size"]) + node.tag].append(
            node
        )
    ...
    tree1_depth_node_old = [
        node for node in tree1.leaves() if node.data["repeat"] == True
    ]

    while len(tree1_depth_node_old) + len(tree1_depth_node_old) > 0:
        tree1_depth_node = []
        tree1_depth_node_id = []
        tree1_depth_node_old_repeat = [
            node for node in tree1_depth_node_old if node.data["repeat"] == True
        ]
        for child in tree1_depth_node_old_repeat:
            ...
            if parent_node is None:
                continue
            if (
                parent_node.identifier not in tree1_depth_node_id
                and child.data["repeat"] == True
            ):
                ...
                for node2 in node_list:
                    ...
                    if same_node_children(node1, node2, tree1):
                        ...
                        tmp_size = sum(
                            [
                                node.data["size"]
                                for node in tree1.children(node1.identifier)
                            ]
                        )
                        ...
                        tmp_size = sum(
                            [
                                node.data["size"]
                                for node in tree1.children(node2.identifier)
                            ]
                        )
                        tmp_data["size"] = tmp_size
...
def get_subtree_size(tree):
    ...
    all_repeat_node_tmp = [
        ...
        if node.data["repeat"] == True
        and tree.parent(node.identifier).data["repeat"] != True
        and node.data["size"] > 0
    ]
    hash_list = [
        str(node.data["hash"]) + str(node.data["size"]) + node.tag
    ...
    all_repeat_layer_size = sum(
        [
            node.data["size"]
            for node in all_repeat_node
            if node.data["repeat"] == True
            and tree.parent(tree.parent(node.identifier).identifier).identifier == 0
        ]
    )
...
def classfy_subtree(img_list):
    ...
    tree.create_node(
        ...
        identifier=node_id,
        parent=None,
        data={"name": 0, "size": 0, "hash": 0, "repeat": False, "img": None},
    )  # 根节点
        ...
        for file in files:
            ...
            if addr_file_dict.__contains__(file_path_inlayer):
                ...
                if layer_order.index(
                    ...
                    addr_file_dict[file_path_inlayer] = [
                        file_layer,
                        file_size,
                        file_path,
                    ]
                ...
                for file in files:
                    ...
                    if file_size > 0:
                        ...
                        if (
                            ...
                            and dict_all[file_hash + file + str(file_size)][0]["size"]
                            > 0
                        ):
                            unit_class = []
