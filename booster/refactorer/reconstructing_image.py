...
def mp_copy_image(input_list):
    ...
    layer_order_addr = [addr + '/' + one_layer.split('/')[0] + '/' for one_layer in
                        load_json(addr + '/manifest.json')[0]['Layers']]
    for addr in layer_order_addr:
        ...
        shutil.copytree(addr, './output/' + img + '/unit/base/' + addr.split('/')[-2], symlinks=True,
...
def split_image(dict_all, img_list):
    ...
    if TEST_FLAG == True:
        if parallel_flag:
            pool = Pool()
            pool.map(mp_copy_image, zip(img_list, [0 for null_value in range(len(img_list))]))
            pool.close()
            pool.join()
        else:
            for img in img_list:
    ...
    if TEST_FLAG == True:
        ...
        if parallel_flag:
            pool = Pool()
            pool.map(mp_reconstrucuting, zip(img_list, [dict_all for null_value in range(len(img_list))]))
            pool.close()
            pool.join()
        else:
            for img in img_list:
        ...
        if parallel_flag:
            pool = Pool()
            pool.map(mp_main_tar, zip(img_list, [0 for null_value in range(len(img_list))]))
            pool.close()
            pool.join()
        else:
            for img in img_list:
