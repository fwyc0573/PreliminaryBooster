    ...
    for each_image in os.listdir(images_path):
        ...
        try:
            ...
            for item in os.listdir(layer_path):
                shutil.move(
                    os.path.join(layer_path, item), metadata_path
                ) if os.path.isfile(os.path.join(layer_path, item)) else 0
        except:
            print("元数据存储位置正确...")
        ...
        for unit_id in unit_dict:
            ...
            print(
                unit_id,
                "(unit) was clipped from  ",
                from_layer,
                "(layer) which order is ",
                pos,
            )
            ...
            basic_dir = (
                layer_path
                if os.path.exists(os.path.join(layer_path, son_layer_uuid))
                else unit_path
            )
            with open(
                os.path.join(basic_dir, son_layer_uuid, "json"), encoding="utf-8"
            ) as file:
        ...
        for item in os.listdir(layer_path):
            ...
            diff_id_list[pos] = "sha256:" + file_hash_coding(
                os.path.join(layer_path, item, "layer.tar"), False
            )
            shutil.move(
                os.path.join(layer_path, item),
                os.path.join(images_path, each_image, "reconstruct"),
            )
        for item in os.listdir(unit_path):
            shutil.move(
                os.path.join(unit_path, item),
                os.path.join(images_path, each_image, "reconstruct"),
            )
        ...
        with open(
            os.path.join(images_path, each_image, "reconstruct", "manifest.json"), "w"
        ) as file:
            file.write(json.dumps(manifest_dict))
        with open(
            os.path.join(images_path, each_image, "reconstruct", config), "w"
        ) as file:
            file.write(json.dumps(config_dict))
        shutil.move(
            os.path.join(metadata_path, "repositories"),
            os.path.join(images_path, each_image, "reconstruct"),
        )
        break
...
def reconstruct_layer_v2():
    ...
    for each_image in os.listdir(images_path):
        ...
        layer_list = [
            item
            for item in os.listdir(unit_path)
            if os.path.isdir(os.path.join(unit_path, item))
        ]
        ...
        for unit_layer in layer_list:
            if unit_index == len(layer_list) - 1:
                layer_json_creat(
                    ...
                    is_last=True,
                    is_first=False,
                    parent_id=layer_list[unit_index - 1],
                    config_json=copy.deepcopy(config_dict),
                )
            elif unit_index == 0:
                layer_json_creat(
                    unit_path,
        ...
        manifest_dict[0]["RepoTags"][0] = (
            "fengyicheng/" + RepoTags if "fengyicheng/" not in RepoTags else RepoTags
        )
        ...
        config_dict["rootfs"]["diff_ids"] = [
            "sha256:" + item for item in layer_list if "sha256:" not in item
        ]
        ...
        if upload_to_hub:
            pro = subprocess.Popen(
                "docker push " + repo_tags, shell=True, stdout=subprocess.PIPE
            )
            pro.wait()
        t3_time = time.time()
        ...
        os.remove(tar_path)
        pro = subprocess.Popen(
            "docker rmi -f " + repo_tags, shell=True, stdout=subprocess.PIPE
        )
        pro.wait()
...
def reconstruct_layer_accelerate():
    ...
    for item in os.listdir(tmp_folder_path):
        layer_exit_list.append(item) if os.path.isdir(
            os.path.join(tmp_folder_path, item)
        ) else 0
    ...
    for index in range(len(layer_list_final)):
        ...
        with open(
            os.path.join(tmp_folder_path, layer_id, "json"), "r", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(tmp_folder_path, layer_id, "json"), "w", encoding="utf-8"
        ) as f:
            f.write(json.dumps(json_dict))

    manifest_dict[0]["Layers"] = [item + "/layer.tar" for item in layer_list_final]
    manifest_dict[0]["RepoTags"] = [
        origin_name + "_tmpImage"
        for origin_name in manifest_dict[0]["RepoTags"]
        if "_tmpImage" not in origin_name
    ]
...
def layer_json_creat(
    basic_path, my_id, is_last, is_first, parent_id=None, config_json=None
):
