...
import requests
import gzip
from utils_images import get_single_layer_from_hub, decompress_layerFile, rm_single_layerFile, layer_list_get, \
    gzip_decompress_rm, layer_json_creat, deploy_tar_creat, get_image_layer_from_hub
...
def v1_thr_downloading(args):
    print("thr_downloading | Thread %s, parent process %s" % (threading.get_ident(), os.getppid()))
    obj, ublob = args
    ...
    time1 = time.time()
    bresp = requests.get('https://{}/v2/{}/blobs/{}'.format(obj["registry"], obj["repository"], ublob),
                         headers=obj["auth_head"], stream=True, verify=False)
...
class BoosterAgent(object):
    ...
    def __init__(self, agent_name="booster-agent", host_node=os.getlogin()):
        ...
        self._changes_sync_dict = {"layer": {"size_update": {}, "rm": []}, "file": {"size_update": {}, "rm": []},
    ...
    def init_local_cache_info_to_master(self):
        ...
        for layer_dict in init_layer_list:
            self.changes_sync_add({"layer": {"size_update": layer_dict}}, "layer", "size_update")
    ...
    def init_daemons(self):
        ...
        parallel_uploading_daemons_thr = multiprocessing.Process(target=self.bro_to_send_daemons)
        ...
        coming_soon_sync_daemons_thr = threading.Thread(target=self.coming_soon_sync_daemons)
        coming_soon_sync_daemons_thr.start()

        deploy_construct_daemons_thr = threading.Thread(target=self.deploy_construct_daemons)
        deploy_construct_daemons_thr.start()

        asyn_cache_update_daemons_pro = threading.Thread(target=self.cache_update_daemons)
        asyn_cache_update_daemons_pro.start()

        parallel_downloading_daemons_pro = multiprocessing.Process(target=self.downloading_daemons)
    ...
    def thr_downloading(self, args):
        ...
        get_size = get_single_layer_from_hub(downloading_dict['ublob'], downloading_dict['image_name'],
                                             downloading_dict['save_path'], downloading_dict['tmp_path'])
        my_id = "sha256:" + downloading_dict['ublob'] if "sha256:" not in downloading_dict['ublob'] else \
        downloading_dict['ublob']

        print(f"thr_downloading | downloading_dict['ublob']: {my_id} -> finish")

        self.coming_soon_update_queue.put(
            {"action": "rm", "granularity": "layer", "my_id": my_id[7:]}) #if my_id[7:] in self.coming_soon_set["layer"] else 0
    ...
    def coming_soon_sync_daemons(self):
        ...
        while True:
            ...
            action, granularity, my_id = my_dict["action"], my_dict["granularity"], my_dict["my_id"],
    ...
    def v1_downloading_daemons(self):
        ...
        while True:
            ...
            for ublob in obj["need_ublob_list"]:
                ...
                task.add_done_callback(thread_exception)
                print(f"downloading_daemons | ublob: {ublob} -> pool.submit | worker_num: {pool._work_queue.qsize()}")
    ...
    def get_layer_from_hub(self, my_dict):  # {"layer_list": layer_list, "image_name": image_name, "image_id": image_id, "work_type": work_type}
        if my_dict['work_type'] == "deploy":
            for my_layer in my_dict["layer_list"]:
                downloading_dict = {"ublob": my_layer, "image_name": my_dict["image_name"],
                                    "save_path": self.__menu_basic_path + "construct/" + my_dict['image_id'] + "/",
    ...
    def trans_layer_to_bro(self, args):
        ...
        result = proxy_uploader(ip_digestList_list[0], "9000", ip_digestList_list[1], ip_digestList_list[2],
    ...
    def thread_callback_sync(self, res):
        ...
        res = res.result()
        self.coming_soon_update_queue.put(
            {"action": "rm", "granularity": "layer", "my_id": res[1]}) if res[1] in \
                                                                                   self.coming_soon_set[
                                                                                       "layer"] else 0
        print(f"thread_callback_sync | bro_trans layer_id = {res[1]} -> coming_soon_set")
    ...
    def decompress_rm_daemons(self):
        ...
        while True:
            ...
            task = pool.submit(decompress_layerFile, (wait_obj[0], wait_obj[1], wait_obj[2], wait_obj[3], wait_obj[4]))
    ...
    def changes_sync_daemons(self, interval=0.2):
        ...
        while True:
            with self._changes_sync_lock:
                if self._changes_sync_dict["layer"]["size_update"] or self._changes_sync_dict["layer"]["rm"] or \
                        self._changes_sync_dict["image"]["size_update"]:
                    result = proxy_changes_syncer("192.168.1.70", "9000", self.host_name, self._changes_sync_dict)
    ...
    def bro_to_send_add(self, ip_digestList_dict):
        for ip in ip_digestList_dict:
            ...
            self._bro_to_send_queue.put([ip, ip_digestList_dict[ip]["layer"], ip_digestList_dict[ip]["file"],
                                         ip_digestList_dict[ip]["trans_id"]])
    ...
    def changes_sync_add(self, tmp_dict, granularity, operation):
        with self._changes_sync_lock:
            if operation == "rm":
                self._changes_sync_dict[granularity][operation] += tmp_dict[granularity][operation]
            elif operation == "size_update":
                for my_id in tmp_dict[granularity][operation]:
                    self._changes_sync_dict[granularity][operation][my_id] = tmp_dict[granularity][operation][my_id]
    ...
    def deploy_construct_daemons(self):
        ...
        while True:
            ...
            task = pool.submit(self.lifecyle_manager,
                               (wait_obj[0], wait_obj[1], wait_obj[2]))
            task.add_done_callback(thread_exception)
    ...
    def cache_update(self, my_dict):
        ...
        image_id, image_name, my_id, layer_path = my_dict["image_id"], my_dict["image_name"], my_dict["my_id"], my_dict[
            "layer_path"]
        # print(f"cache_update | local cache update, layer id is {my_id}.")
        layer_size = int(os.path.getsize(os.path.join(layer_path, "layer.tar")) / 1024 / 1024)
        with self._cache_update_lock:
            ...
            #                                                       size=layer_size)
            do_size_update, remove_back_layer_list = self.arc.set(key=my_id, image_name=image_name, image_id=image_id,
                                                                  size=layer_size)
        ...
        if do_size_update:
            ...
            shutil.move(os.path.join(self.__menu_basic_path, "construct", image_id, my_id), target_path)
            ...
            self.changes_sync_add({"layer": {"size_update": {my_id: layer_size}}}, "layer", "size_update")
        last_time = time.time()
        if len(remove_back_layer_list) > 0:
            self.local_to_del_add({"layer": remove_back_layer_list})
            self.changes_sync_add({"layer": {"rm": remove_back_layer_list}}, "layer", "rm")
            # print(f"cache_update | remove time = {time.time()-last_time}")
        self.cache_updating_now_dict[image_id].remove(my_id)
        print(f"cache_update | layer: {my_id}, whole use time = {time.time()-start_time}")
    ...
    def lifecyle_manager(self, args):
        ...
        for metadata in metadata_list:
            print(f"add path: ", os.path.join(self.__menu_basic_path, "construct", image_id, metadata))
            tar.add(os.path.join(self.__menu_basic_path, "construct", image_id, metadata),
                    arcname=metadata)  # just add the file use "arcname=metadata"
        ...
        while len(not_ready_list):
            ...
            for my_id in not_ready_list:
                my_layer_construct_dir = os.path.join(self.__menu_basic_path, "construct", image_id, my_id)
                if os.path.exists(my_layer_construct_dir) and len(os.listdir(my_layer_construct_dir)) > 0:  # 从tmp mv
                    layer_json_creat(construct_path, my_id, layer_list, image_id)  # 写入json
                    tar.add(os.path.join(self.__menu_basic_path, "construct", image_id, my_id), arcname=my_id)
                    ...
                    #                                               my_id)})
                    self.cache_update_queue.put({"image_id": image_id, "image_name": image_name, "my_id": my_id,
                                                 "layer_path": os.path.join(self.__menu_basic_path, "construct",
                                                                            image_id, my_id)})
                ...
                elif os.path.exists(os.path.join(self.__menu_basic_path, "layer", my_id)):  # 缓存中找
                    start_time = time.time()
                    os.mkdir(construct_path + my_id)  # make layer dir
                    src = os.path.join(self.__menu_basic_path, "layer", my_id, "layer.tar")
                    dst = os.path.join(self.__menu_basic_path, "construct", image_id, my_id, "layer.tar")
                    os.symlink(src, dst, target_is_directory=False)
                    layer_json_creat(construct_path, my_id, layer_list, image_id)  # 写入json
                    # tar.add(os.path.join(self.__menu_basic_path, "construct", image_id, my_id), arcname=my_id, dereference=True)
                    for root, dirs, files in os.walk(os.path.join(self.__menu_basic_path, "construct", image_id, my_id)):
                        for file in files:
                    ...
                    not_ready_list.remove(my_id)
                    # coming_soon?
                    print(f"lifecyle_manager | cache add to tar, use time = {time.time() - start_time}")
                else:
                    if my_id not in self.coming_soon_set["layer"]:  # layer_list里头取的，这儿先不考虑file了
                        self.get_layer_from_hub({"layer_list": [my_id], "image_name": image_name, "image_id": image_id,
                                                 "work_type": 'deploy'})
                        self.coming_soon_update_queue.put({"action": "add", "granularity": "layer", "my_id": my_id})
                    continue
        ...
        self.changes_sync_add({"image": {"size_update": {image_id: image_tar_size}}}, "image", "size_update")
        ...
        proxy_notify_kubectl_delete("192.168.1.70", '9000', self.host_name, image_id, image_name, pull_and_tar_time, self.arc._cache_hit)
