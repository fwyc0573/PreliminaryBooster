/*
	Solution for accelerating the image layers extracting in  (still testing...):
...
func ApplyLayerWithOpts(ctx context.Context, layer Layer, chain []digest.Digest, sn snapshots.Snapshotter, a diff.Applier, opts []snapshots.Opt, applyOpts []diff.ApplyOpt) (bool, error) {
	...
	if _, err := sn.Stat(ctx, chainID); err != nil {
		...
		if err := V0ApplyLayers(ctx, []Layer{layer}, append(chain, layer.Diff.Digest), sn, a, opts, applyOpts); err != nil {
...
// make db maintain diff.Digest rather than chainID
// TODO: Test container startup and DB reading (absolutely a wrong way, buy what happen?)
func v1ApplyLayers(ctx context.Context, layers []Layer, chain []digest.Digest, sn snapshots.Snapshotter, a diff.Applier, opts []snapshots.Opt, applyOpts []diff.ApplyOpt) error {
	var (
		//parent  = identity.ChainID(chain[:len(chain)-1])
		//chainID = identity.ChainID(chain)
		layerDiffDigest = chain[len(chain)-1]  // get layer.Diff.Digest for apply judge
	...
	var parentDiffDigest string  // get layer.Diff.Digest for apply judge
	if len(chain) == 1 {
		parentDiffDigest = chain[0]
	} else {
		parentDiffDigest = chain[len(chain)-2]
	}

	for {
		key = fmt.Sprintf(snapshots.UnpackKeyFormat, uniquePart(), layerDiffDigest)

		// Prepare snapshot with from parent, label as root
		mounts, err = sn.Prepare(ctx, key, parentDiffDigest.String(), opts...)
		if err != nil {
			if errdefs.IsNotFound(err) && len(layers) > 1 {
				if err := v1ApplyLayers(ctx, layers[:len(layers)-1], chain[:len(chain)-1], sn, a, opts, applyOpts); err != nil {
					if !errdefs.IsAlreadyExists(err) {
						return err
					}
				}
	...
	if err = sn.Commit(ctx, layerDiffDigest.String(), key, opts...); err != nil {
...
// We add layer.Digest (contentDigest) to meta.db as a key like "contentDigest": 2719033213..asdffff.
// ChainID still is used to prepare (snapshot dir creating), but before extracting,
// a compare between layer.Digest will be performed. In this manner, we can reduce
// We reduce redundency by hard links.
// TODO: Test container startup and DB reading
func v2ApplyLayers(ctx context.Context, layers []Layer, chain []digest.Digest, sn snapshots.Snapshotter, a diff.Applier, opts []snapshots.Opt, applyOpts []diff.ApplyOpt) error {
	var (
		parent  = identity.ChainID(chain[:len(chain)-1])
		chainID = identity.ChainID(chain)
		layerDiffDigest = chain[len(chain)-1]  // get layer.Diff.Digest for apply judge
	...
	for {
		...
		if err != nil {
			if errdefs.IsNotFound(err) && len(layers) > 1 {
				if err := v2ApplyLayers(ctx, layers[:len(layers)-1], chain[:len(chain)-1], sn, a, opts, applyOpts); err != nil {
					if !errdefs.IsAlreadyExists(err) {
						return err
					}
				}
...
// applyLayers make extract depend on chainID rather than digest
// FIXME: layer redundance appears because of chainID use; yet we think read-only snashopt can void this bad situation by snapshot oringal diff.Digest compare
